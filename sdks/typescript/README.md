...
Node.js 22+ is required. The wrapper selects an optional prebuilt runtime package for the current OS and CPU architecture. Each runtime package contains the Rust executable and matching llama.cpp/GGML shared libraries; installation runs no compilation or download scripts. Model weights are supplied separately. These packages are prepared for distribution but have not been published to npm.
...
For end users, install the wrapper and the platform runtime tarballs generated by the build workflow. For example, on Linux x64:

```sh
npm install ./l2s1-node-0.1.1.tgz ./l2s1-runtime-linux-x64-0.1.1.tgz
```

After the packages are published, `npm install @l2s1/node` selects the runtime through optional dependencies. Keep optional dependencies enabled. Runtime and wrapper versions must match.
