...
def measure(args, mode, groups):
    ...
    command = [str(args.binary.resolve()), "--model", str(args.model.resolve()),
               ...
               "--execution-mode", mode, "--parallel-width", "4",
...
def main():
    ...
    runs = {mode: measure(args, mode, groups) for mode in modes}
    ...
    summary = {"protocol": "Identical 4-image HTTP groups, one independent decision per image; RTX 3080 CUDA, context4096, token batch256, threads4, width4; one warmup group then repeated full120 passes per mode. Filenames and labels excluded.",
