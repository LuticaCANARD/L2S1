...
impl WebGpuBackend {
    ...
    async fn new_impl(cache_data: Option<&[u8]>) -> Result<Self, GpuError> {
        ...
        // wgpu 24's Naga rejects `enable f16` on this Metal adapter even when
        // the feature is advertised. Keep the portable f32 kernels enabled.
        ...
        // wgpu 24's Naga also rejects `enable subgroups` on this adapter.
        let has_subgroups = false;
        if has_subgroups {
