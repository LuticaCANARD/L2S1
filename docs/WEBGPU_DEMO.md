...
[English](en/WEBGPU_DEMO.md) · [한국어](ko/WEBGPU_DEMO.md) · [日本語](ja/WEBGPU_DEMO.md)

[English index](en/README.md) · [한국어 색인](ko/README.md) · [日本語索引](ja/README.md)

`/webgpu`는 Qwen3 0.6B ONNX를 브라우저에서 실제 실행합니다. 저장된 예시나 합성 점수를 추론 결과로 표시하지 않습니다. 브라우저 모드의 입력과 생성된 생각 토큰은 Web Worker 안에서 처리하며 서버 추론 API로 보내지 않습니다. 페이지를 열 때 모델을 자동 다운로드하지 않습니다.
...
추론 위치에서 **로컬 서버 · 큰 GGUF 모델**을 선택할 수 있습니다. 같은 상태·질문·수락 기준을 사용하고, 분석 버튼을 누를 때 입력을 로컬 서버로 보냅니다. 모델 연결은 capability 조회만 수행합니다. 결과에는 실제 서버 모델·런타임을 표시하며, 요청한 수락 기준과 다른 기준을 반환하는 서버의 결과는 거부합니다. 서버가 지원하는 경우에만 thinking을 선택할 수 있습니다. 실패 문구는 로컬 HTTP API가 지원하는 코드만 전달합니다.

이 옵션에는 localhost에서 실행한 웹앱과 모델 서버가 필요합니다. 기존 GGUF 파일의 경로를 지정해 서버를 실행합니다. 모델 파일은 저장소에 포함되지 않습니다.
...
다른 터미널에서 `web`의 로컬 개발 서버를 실행하고, `/webgpu`에서 로컬 서버를 선택한 뒤 **로컬 모델 연결**, **로컬 모델로 분석**을 누릅니다. 기본 모델 서버 주소는 `http://127.0.0.1:8082`이며, 웹앱을 시작할 때 `L2S1_QUALITY_URL`로 변경할 수 있습니다. **요청 대기 중단**은 브라우저의 응답 대기를 취소합니다.

2026-09-26의 추가 진단에서 4시간 물류 입력은 브라우저 0.6B q4에서 `medium`을 최고 후보로 평가하고 보류했습니다. 답변 경계 제거·JSON 프롬프트·비교 조건 안내·q8 실행도 이 오답을 해결하지 못해 본 코드에 적용하지 않았습니다. 로컬 GGUF 메타데이터가 `qwen35 27B IQ2_XXS`인 모델은 동일 입력의 세 질문에서 `chilled`, `true`, `high`를 모두 선택했습니다. 출고 우선순위의 후보 상대 확률은 **0.9788991935**, 후보 코드 질량은 **0.9848324846**입니다. 기본 정책 **0.8 / 0.05**를 유지했고 CPU로 실행했습니다. 로컬 파일명은 위 명령에 표시한 이름이며 실제 아키텍처는 `qwen35`입니다.

추가 경계 확인에서 6·7·24·25시간의 최고 후보는 모두 정답이었습니다. 정확히 6시간과 24시간은 상대 확률이 0.8보다 낮아 보류했고, 7시간과 25시간은 정답을 선택했습니다. choice 후보 순서를 바꿔도 `chilled`를 선택했습니다.

이는 특정 물류 예시의 재현 확인이며 일반 정확도나 운영 품질 보장이 아닙니다. 모델·양자화·런타임·프롬프트가 달라 통제된 모델 비교나 속도 우위를 주장하지 않습니다. 브라우저 모델의 작은 후보 질량은 화면에서 `0.00%`로 반올림되는 대신 `<0.01%`로 표시합니다.

현재 네이티브 HTTP 서버는 상태·질문만 받고 시작 시 설정한 정책을 적용합니다. 요청별 정책 지원을 알리지 않은 서버에는 이 계약을 사용하며 thinking을 비활성화하고 화면 기준과 다른 응답 정책은 거부합니다. 정책을 바꾸려면 서버를 화면과 같은 `--min-top-probability`, `--min-candidate-mass` 값으로 시작하세요. `request_policy.supported`를 알리는 새 서버에는 요청 정책과 지원 실패 문구를 전달합니다. 현재 서버의 사용자 실패 문구는 원래 이유 코드를 유지하며 클라이언트에서 표시합니다.
...
`http://127.0.0.1:5173/webgpu`를 엽니다. 별도 L2S1 추론 서버는 로컬 서버 모드에서만 필요합니다. WebGPU 모델을 실행하려면 지원 브라우저·어댑터와 모델을 내려받을 네트워크가 필요합니다.
