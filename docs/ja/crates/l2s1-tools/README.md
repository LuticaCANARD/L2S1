...
Rust に置き換えた Python の準備・評価・レポート実行器 21 個と、対応する Python 実装専用テスト 7 個を削除しました。`kaggle_ag_news.py`、`calibrate_ag_news.py`、`prepare_accuracy_study.py` は Python 学習コードがハッシュ・校正・固定データ検証のヘルパーを使うため保持します。それらのテスト、PyTorch 学習・診断、現在の画像 HTTP 実行器も維持します。過去のスクリプトは Git 履歴に残ります。新しいデータ準備・ベンチマーク・レポートには Rust を使ってください。

修正された 正解率 スタディ シードは、すべての 18 凍結ファイルをバイト単位で再現します。 AG ニュース、航空会社、判断 微調整、出力 ヘッド、およびインテントの準備が、既存のローカル アーティファクトに対してチェックされました。 JevBench および Laya の準備と保存された予測スコアも、ピン留めされた公開データに対してチェックされました。これらのチェックでは、新しい CLI の新しいモデル推論、ネットワーク ダウンロード、または GPU の実行は確立されません。これらのパスには、対応するホスト ファイルとモデル ファイルが必要です。
