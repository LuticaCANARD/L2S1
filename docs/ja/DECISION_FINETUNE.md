...
- 出典: `scripts/kaggle_airline.py` が作成した、固定された Kaggle Twitter US Airlines Sentiment アーカイブ。データセットのテキストと重みは無視された `results/` のままです。このリポジトリによって再配布されることはありません。
...
1. `scripts/prepare_decision_finetune.py` はデータとプロトコルをフリーズします。
...
4. `scripts/report_decision_finetune.py` は、校正 のみの温度に適合し、保持されたメトリクスとオプション注文診断をレポートします。
