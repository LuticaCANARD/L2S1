...
Node.js 22+가 필요합니다. 래퍼는 현재 OS 및 CPU 아키텍처에 대해 사전 구축된 선택적 런타임 패키지를 선택합니다. 각 런타임 패키지에는 Rust 실행 파일과 일치하는 llama.cpp/GGML 공유 라이브러리가 포함되어 있습니다. 설치 시 컴파일이나 다운로드 스크립트가 실행되지 않습니다. 모델 중량은 별도로 제공됩니다. [0.1.1 버전](https://www.npmjs.com/package/@l2s1/node/v/0.1.1)을 npm에 게시했습니다.
...
`npm install @l2s1/node@0.1.1`은 선택적 종속성을 통해 런타임을 선택합니다. 선택적 종속성을 활성화된 상태로 유지하세요. 런타임 및 래퍼 버전이 일치해야 합니다.
