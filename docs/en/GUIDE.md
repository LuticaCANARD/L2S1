...
[English](GUIDE.md) · [한국어](../ko/GUIDE.md) · [日本語](../ja/GUIDE.md)
...
Start with the README for the shortest path to a first decision.
...
Load a vision-capable chat GGUF with its matching multimodal projector GGUF (`mmproj`). The projector encodes a still image through llama.cpp `libmtmd`; L2S1 then scores the same typed options from the resulting next-token logits. `LlamaBackend::load_vision_projector(path)` and `LlamaBackend::decide_vision(&request, image_bytes)` expose the Rust API. `LlamaBackend::decide_vision_batch(&requests, &images)` keeps each request's state and image independent. Existing text requests still use `decide`.
...
For native image batching, launch the llama.cpp listener with `--execution-mode parallel --parallel-width 4` and the matching `--mmproj`. Send four media items in one request and give each independent decision its own `media_ids: ["image_id"]`. The runtime encodes compatible projector chunks in a batch and evaluates each image prompt in its own decoder sequence and KV stream; request grouping alone does not enable this. `GET /v1/capabilities` reports whether parallel image execution is enabled. Parallel responses include `backend.details.vision_batch` with the latest native wave's projector and decoder counters. The ordinary `fresh` default keeps the prior serial behavior. Parallel scoring may differ from fresh scoring, so compare decisions and probabilities on the target model. `--parallel-context-dynamic` sizes each KV stream for the wave's longest image prompt plus token batch headroom. Projector layouts that the native scheduler cannot safely batch return an explicit error.

Native vision batching is a supported execution option for compatible non-recurrent, non-hybrid vision models with at most 26 answer options. The [TrashNet batch4 measurement](benchmarks/trashnet-vision-20260925/REPORT.md#native-four-image-batching-2026-09-26) verified real decoder batching and reduced processing time on RTX 3080, but all three tested CUDA checkpoints failed the existing numerical equivalence criterion. Keep fresh execution when preserving its scores and decisions is required.
...
| llama.cpp | Models supported by the pinned llama.cpp revision, including compatible Bonsai and Qwen GGUF | Supported model with matching `mmproj` | Existing fresh, prefix-reuse, state-restore, parallel contracts; vision supports fresh and opt-in parallel | LoRA, calibration, output heads, diagnostics, HTTP |
...
For Rust, construct the backend with `ComputeOptions::vision_optimized()`, load its matching projector, and call `backend.enable_vision_optimizations()?`. A resident backend retains prepared prompts; image embeddings are reused only inside one native wave. The candidate-mass gate still uses the complete vocabulary. Duplicate KV clearing is skipped globally when the native memory is already clean; Rust-only failures and native failures still invalidate state.

Exact preparation caching, compact evidence transfer and dirty-only KV clearing are components of this profile. Their isolated checks preserve scores at an unchanged compute configuration; combining them with parallel decoding and Flash Attention still changes predictions. The serial component experiment did not establish a speedup. The profile is supported for compatible GPU vision models; decoder/projector batch shapes and attention kernels can change predictions. It supports non-recurrent, non-hybrid models and at most 26 answer options. Prefix reuse/shared-state sessions and snapshot restoration remain separate execution strategies; independent images cannot share their image KV state. See the [TrashNet measurements](benchmarks/trashnet-vision-20260925/REPORT.md).
...
- **Compact evidence:** retain candidate logits and the full-vocabulary normalizer while avoiding the complete native-host-to-Rust logits copy. llama.cpp still computes the full vocabulary and makes it available on the host. This is not GPU-side reduction or output-head elimination. It supports text fresh/prefix-reuse and image fresh/parallel execution without a learned output head and has a distinct calibration identity.
