...
[English](REPORT.md) · [한국어](../../../ko/benchmarks/bonsai-state-restore-20260925/REPORT.md) · [日本語](../../../ja/benchmarks/bonsai-state-restore-20260925/REPORT.md)
...
The separate [three-decision CUDA conformance result](../../../../benchmarks/bonsai-state-restore-20260925/conformance.json) covers binary, choice and ordinal decisions on Bonsai. State restoration loaded its 167,384,364-byte snapshot twice, with zero option-probability and candidate-mass difference and no changed top choice or accepted value versus fresh. A zero-byte limit again produced `snapshot_memory_budget` and zero restores. This checks another prompt shape; it is not a labeled accuracy set.

<a id="reproduction-and-limits"></a>
## Reproduction and limits

On a CUDA host with the same GGUF, run the ignored regression using `L2S1_BONSAI_MODEL=/path/to/Bonsai-27B-Q1_0.gguf cargo test --release --locked --features llama-cuda --test state_restore_bonsai -- --ignored --nocapture`. Run the other contract with `L2S1_CONFORMANCE_MODELS=/path/to/Bonsai-27B-Q1_0.gguf SKID_CUDA=1 cargo test --release --locked --features llama-cuda --test conformance -- --ignored --nocapture`. For the CLI diagnostics, use the flags above with `--input` pointing to the linked request and add `--diagnostics`. The full benchmark outputs and test logs remain on the measured host under `~/personal/skid/state-restore-stable-20260925/results/`.
