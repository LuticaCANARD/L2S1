...
[English](README.md) · [한국어](../../../ko/crates/l2s1-tools/README.md) · [日本語](../../../ja/crates/l2s1-tools/README.md)
...
The Rust commands replace 21 redundant Python preparation, evaluation and reporting entrypoints; their seven Python-only test files were removed with them. `kaggle_ag_news.py`, `calibrate_ag_news.py` and `prepare_accuracy_study.py` remain because the Python training code imports their hashing, calibration and frozen-data validation helpers. Their tests, PyTorch training/probes, current vision HTTP runners are maintained. Historical script versions remain in Git history. Use Rust for new data preparation, benchmarking and reports.

The fixed accuracy-study seed reproduces all 18 frozen files byte-for-byte. AG News, airline, decision fine-tuning, output-head, and intent preparation were checked against existing local artifacts. JevBench and Laya preparation and saved-prediction scoring were also checked against pinned public data. These checks do not establish fresh model inference, network download, or GPU execution for the new CLI; those paths require the corresponding host and model files.
