...
[English](VERIFICATION.md) · [한국어](../ko/VERIFICATION.md) · [日本語](../ja/VERIFICATION.md)
...
The pinned llama.cpp revision is `3d82ef62d47fd74e18f36c5eccbdcf965b617b17`. CMake FetchContent downloads it on the first default build and verifies the archive hash. The sys dependency builds matching source, headers and shared libraries. CPU and CUDA builds run on Linux; `llama-metal` builds the same native API with a Metal device on macOS. Set `L2S1_LLAMA_CPP_SOURCE` to use a local checkout for offline checks or to test another upstream revision.
...
On a Mac, build both Metal routes with `cargo build --release --locked --features wgpu --bin l2s1-wgpu` and `cargo build --release --locked --features llama-metal --bin l2s1`. Run the first with `WGPU_BACKEND=metal --require-metal` for Gemma 4 and the second with `--device metal` for general GGUF models. Real Metal inference and equivalence need a Mac with the matching GGUF files. For Gemma 4 request-local mode equivalence, set `L2S1_WGPU_GEMMA_MODEL` and `L2S1_WGPU_GEMMA_MMPROJ`, then run `cargo test --release --locked --features wgpu --test wgpu_execution -- --ignored`.
